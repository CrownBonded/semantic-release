...
async function run(options, plugins) {
  ...
  // TODO
  // Find all releases on current branch that need to be added to a release:
  // - Tags that are associated with the channel of a higher branch but not with the current one
  //
  // For each release:
  //  - Verify if in range of the branch
  //  - Create tag
  //  - Call addChannel plugin
  //
  // Add release to list passed to success ? Imply to find commit for each release
  //
  // Then run normal process for releasing. Just add verif in range.
  //
  ...
  const commits = await getCommits(lastRelease.gitHead, branch.name, logger);
